    ...
    def run(self):
        ...
        while True:
            ...
            if wcfg.cmd == "bcast_g_c1":
                ...
                self.stats.transfered_bits_by_node[wcfg.worker_id, iteration] = ncfg.D * ncfg.component_bits_size
                # On that "c1" mode to evaluate gradient we call oracle number of times how much data is in local "full" batch
                self.stats.fi_grad_calcs_by_node[wcfg.worker_id, iteration] = len(wcfg.train_set)
            ...
            if wcfg.cmd == "bcast_g_c0":
                ...
                for t, (inputs, outputs) in enumerate(minibatch_loader):
                    ...
                    loss.backward()  # compute the gradient (backward-pass)
                    t5 = time()
                    # dbgprint(self.print_lock, self.wcfg, f'For MB cat: {t1 - t0:.3f}; For rest: {t5 - t1:.3f}')
                ...
                for p in model.parameters():
                    ...
                    p.grad = None
                # dbgprint(self.print_lock, self.wcfg, f'For Grad clone -> g_batch_next: {time() - t0:.3f}')
                ...
                t1 = time()
                # dbgprint(self.print_lock, self.wcfg, f'For sub params: {t1 - t0:.3f}')
                ...
                # dbgprint(self.print_lock, self.wcfg, f'Delta offsetting: {time() - t1:.3f}')
                # t1 = time()

                delta_flatten = wcfg.compressor.compressVector(delta_flatten)
                # dbgprint(self.print_lock, self.wcfg, f'Compressing delta_flatten: {time() - t1:.3f}')
                # t1 = time()
                ...
                # dbgprint(self.print_lock, self.wcfg, f'Delta offsetting p2: {time() - t1:.3f}')
                # t1 = time()
                ...
                self.stats.transfered_bits_by_node[
                    wcfg.worker_id, iteration] = wcfg.compressor.last_need_to_send_advance * ncfg.component_bits_size
                self.stats.fi_grad_calcs_by_node[wcfg.worker_id, iteration] = 2 * ncfg.batch_size_for_worker
                iteration += 1

                wcfg.cmd_output_ready.release()

                # dbgprint(self.print_lock, self.wcfg, f'For end of work: {time() - t1:.3f}')
